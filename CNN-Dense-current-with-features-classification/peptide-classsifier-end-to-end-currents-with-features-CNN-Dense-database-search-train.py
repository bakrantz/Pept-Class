# Peptide classifier using translocation event current sequences and features - Training
# Current sequences of translocation events are fed into three layered Conv1D CNN input branch
...
def create_peptide_classifier_model(input_sequence_length, num_features, num_peptides):
    """
    Creates a dual-input CNN-Dense model for end-to-end peptide classification.
    One input branch processes translocation event current sequences with a 3-layer CNN.
    ...
    Args:
        input_sequence_length (int): The fixed length of the input current sequences (timepoints).
    ...
    # --- Input Branch 1: Translocation Event Current Sequences (CNN Branch) ---
    # Define the input layer for the current sequences
    sequence_input = Input(shape=(input_sequence_length, 1), name='sequence_input')

    # Add Gaussian Noise for regularization on the sequence input
    ...
    # The number of units here can be tuned; starting with 32 units.
    # This layer helps the model learn relationships within the features before concatenation.
    features_dense = layers.Dense(32, activation='relu')(features_input)
    # Optional: Add BatchNormalization and Dropout here if features are complex or high-dimensional
    # features_dense = layers.BatchNormalization()(features_dense)
    # features_dense = layers.Dropout(0.2)(features_dense)

    # --- Concatenate Both Branches ---
    # Combine the outputs of the CNN branch and the features branch
    concatenated = layers.concatenate([cnn_output, features_dense], name='concatenation_layer')

    # --- Classifier Head (Dense Layers) ---
    # Dense Layer 1: Processes the combined features
    y = layers.Dense(64, activation='relu')(concatenated)
    # Dropout as per your previous model
    y = layers.Dropout(0.45)(y)

    # Output Layer: Final dense layer for classification
...
def load_translocation_data_from_database(
    ...
    # Separate currents and features from the downsampled tuples
    all_current_sequences = [np.array(event_tuple[0]['currents'], dtype=np.float32) for event_tuple in downsampled_events_and_features]
    ...
    return all_current_sequences, all_features_np, all_labels_np
...
def evaluate_model(model, x_seq_test, x_feat_test, y_test_one_hot, peptide_names_list, best_model_weights_filepath):
    ...
    Args:
        ...
        x_test: Test data (padded translocation event current sequences).
...
if __name__ == "__main__":
    ...
    desired_processing_params = {
        ...
        'min_event_duration_ms': 20, # Events shorter than this time in ms are not included
    ...
    all_current_sequences, all_features_np, all_labels_np = load_translocation_data_from_database(
        peptide_names_list,
        peptide_labels_encoding,
        desired_processing_params,
    ...
    y_train, y_test = train_test_split(
        all_current_sequences, # First X input
    ...
    # Assuming x_seq_train is a list of your current sequences scaled between 0 and 1
    # max_train_sequence_length is determined from your training data
    x_seq_train_padded = pad_sequences(x_seq_train, maxlen=max_effective_sequence_length, padding='post', dtype='float32', value=-1.0)
    x_seq_test_padded = pad_sequences(x_seq_test, maxlen=max_effective_sequence_length, padding='post', dtype='float32', value=-1.0)

    # Reshape for Conv1D input (add a channel dimension: (num_samples, seq_length, 1))
    ...
    print(f"NaNs in x_seq_train_padded: {np.isnan(x_seq_train_padded).any()}")
    print(f"NaNs in x_seq_test_padded: {np.isnan(x_seq_test_padded).any()}")

    # --- 7. Training Callbacks ---
    model_name = "CNN_Dense_current_seqs_and_features_guesthost_5ms" # Descriptive model name
    ...
    class_weights = {0: 1.05, 1: 1.05, 2: 1.0, 3: 1.2, 4: 0.8, 5: 0.7, 6: 1.0} # First weights scheme (best result)
    # class_weights = {0: 1.15, 1: 1.05, 2: 1.0, 3: 1.25, 4: 0.8, 5: 0.7, 6: 1.0} # Second weights scheme (poor result)
    # class_weights = {0: 1.05, 1: 1.05, 2: 1.0, 3: 1.25, 4: 0.8, 5: 0.7, 6: 1.0} # Third weights scheme (intermediate result)
    # class_weights = {0: 1.00, 1: 1.05, 2: 1.0, 3: 1.20, 4: 0.8, 5: 0.7, 6: 1.0} # Fourth weights scheme (intermediate result)
    ...
    history = model.fit(
        ...
        [x_seq_train_padded, x_feat_train], # Training data: [current sequences, features]
    ...
    # The best weights are already saved by ModelCheckpoint. This saves the final weights
    # (which might not be the best if early stopping triggered before the last epoch).
    # It's good practice to save both.
