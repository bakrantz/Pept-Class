# Peptide classifier with event-level features only - Training
...
def load_peptide_data_with_features(peptide_data_paths, peptide_labels_encoding, test_size=0.2, random_state=42, downsample_fractions=None):
    ...
    all_features = [
        [
            # event['entropy'], # Not a strong contributing feature 
...
if __name__ == "__main__":
    ...
    input_shape_features = (12,) # Define input shape for features
    ...
    # Standardize/scale the features in train and test sets 
    ...
    peptide_classifier_model = create_peptide_classifier_model(num_peptides=num_peptides, sequence_length=max_sequence_length, num_features=12, use_tcn=True, use_lstm=False)

    print(peptide_classifier_model.summary()) # Print model summary to console

    # --- 4. Training Callbacks ---
    model_name = "peptide_classifier_with_12_event_level_features_and_TCN_peptides_A_to_G" # More descriptive model name
    ...
    scaler_filepath = f'./models/{model_name}_feature_scaler.pkl' # Feature standardization scaler object filepath 
    ...
    history = peptide_classifier_model.fit(
        [x_train_sequences, x_train_features_scaled], # Training data (sequences and features)
    ...
    evaluate_model(peptide_classifier_model, [x_test_sequences, x_test_features_scaled], test_labels_one_hot, peptide_names_list, best_model_weights_filepath)
    ...
    print(f"Fitted StandardScaler saved to: {scaler_filepath}")
