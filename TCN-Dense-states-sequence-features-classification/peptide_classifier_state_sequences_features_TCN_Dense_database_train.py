...
# --- Model Definition (User's provided function) ---
def create_peptide_classifier_model(num_peptides, sequence_length, num_features, use_tcn=True, use_lstm=False):
    """
    ...
    if use_tcn:
        # TCN layers from the keras-tcn library
    ...
    # Keras metrics are typically capitalized (Accuracy, Precision, Recall)
...
if __name__ == "__main__":
    ...
    desired_processing_params = {
        ...
        'min_event_duration_ms': None, # event-length cut-off filter
    ...
    model_name = "peptide_classifier_TCN_Dense_with_features_guesthost_20ms" # Descriptive model name
    ...
    # Call the new model function with appropriate flags
    model = create_peptide_classifier_model(
        num_peptides=num_peptides,
        sequence_length=max_effective_sequence_length,
        num_features=num_features,
        use_tcn=True,
        use_lstm=False # Set to False for TCN-Dense
    )
    model.summary()
