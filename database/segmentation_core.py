...
# (k) Calculate Skewness on Current Sequence as a Scalar
def calculate_skewness(data):
    """
    Calculates the skewness of a data distribution.
    Skewness is a measure of the asymmetry of the probability distribution.
    """
    if len(data) < 3: # Need at least 3 points to compute skewness
        return 0.0
    return stats.skew(data)

# (l) Calculate Kurtosis on Current Sequence as a Scalar
def calculate_kurtosis(data):
    """
    Calculates the Fisher's kurtosis of a data distribution.
    Kurtosis is a measure of the "tailedness" or "peakedness" of the distribution.
    """
    if len(data) < 4: # Need at least 4 points to compute kurtosis
        return 0.0
    # The default in scipy is Fisher's kurtosis, which is what we want (excess kurtosis)
...
def compute_event_level_features(event_currents, state_sequences, open_state):
    ...
    now including skewness and kurtosis of the current signal.
    ...
    for i in range(len(state_sequences)):
        ...
        # We'll use the existing num_states calculation for the 'Number of Substates' feature
        ...
        # --- NEW FEATURES ---
        current_skewness = calculate_skewness(event_current)
        current_kurtosis = calculate_kurtosis(event_current)
        ...
        event_data = {
            ...
            # --- NEW SCALAR FEATURES ---
            'current_skewness': current_skewness, # Scalar
            'current_kurtosis': current_kurtosis, # Scalar
...
def compute_global_features(translocation_events_data, open_state):
    """
    ...
    # --- ADDED NEW GLOBAL AVERAGES ---
    average_current_skewness = calculate_mean_for_key(translocation_events_data, 'current_skewness')
    average_current_kurtosis = calculate_mean_for_key(translocation_events_data, 'current_kurtosis')

    print(f"Computed global averages: Length={average_event_length:.2f}, Entropy={average_event_entropy:.2f}, Skewness={average_current_skewness:.2f}, Kurtosis={average_current_kurtosis:.2f}, Avg Num States={average_num_states:.2f}")
    ...
    for event in translocation_events_data:
        ...
        # --- ADDED NEW GLOBAL AVERAGES TO EACH EVENT ---
        event['average_current_skewness'] = average_current_skewness
        event['average_current_kurtosis'] = average_current_kurtosis
...
def prepare_ml_dl_data(translocation_events_data, open_state): # Accept open_state
    ...
    # --- UPDATED: Added new scalar event keys ---
    ...
    # --- UPDATED: Added new scalar global keys ---
    scalar_global_keys = [
        ...
        'average_current_skewness', 'average_current_kurtosis'
    ]
