...
def get_ln_survival_and_fit_matrix(dwell_matrix):
    """
    Processes a dwell time matrix to calculate the ln survival function for each
    transition and performs a linear fit.
    ...
    ln_survival_matrix = np.empty((rows, cols), dtype=object)

    for i in range(rows):
        for j in range(cols):
            dwells = dwell_matrix[i, j]
            
            # The corrected line to handle both empty lists and NumPy arrays
            if dwells is None or len(dwells) < 2:
                # We need at least 2 data points for a linear fit.
                ln_survival_matrix[i, j] = None
                continue

            times, ln_survival = ln_survival_func(dwells)
            
            # Skip if ln_survival_func returned None due to insufficient data
            if times is None:
                ln_survival_matrix[i, j] = None
                continue

            # Perform a linear fit on the log-survival data.
            # The slope of this fit is -1/tau, where tau is the mean dwell time.
            # The rate constant k = 1/tau = -slope.
    ...
    return ln_survival_matrix

def plot_ln_survival_and_residuals(results_matrix, nanopore_name, peptide_name, plot_dir='./plots/'):
    """
    Generates and saves two pages of plots:
    1. Log-survival plots with linear fits.
    2. Residuals of the linear fits.

    Args:
        results_matrix (numpy.ndarray): The matrix of dictionaries from
                                        get_ln_survival_and_fit_matrix().
    ...
    if not data_transitions:
        print("No transitions with sufficient data to plot.")
        return
    ...
    # Flatten the axes array for easy iteration if the grid is not 1x1
    if num_plots > 1:
        axes1 = axes1.flatten()
    else:
        axes1 = [axes1]
    ...
    fig1.suptitle(f'Log-Survival Plots with Linear Fits for {peptide_name} peptide via {nanopore_name} nanopore', fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    survival_file = f'{nanopore_name}_{peptide_name}_log_survival_plots.png'
    fig1.savefig(os.path.join(plot_dir, survival_file))
    print(f"Log-survival plots saved to '{plot_dir}{survival_file}'")
    ...
    fig2.suptitle('Residuals of Linear Fits', fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    residual_file = f'{nanopore_name}_{peptide_name}_residuals_plots.png'
    fig2.savefig(os.path.join(plot_dir, residual_file))
    print(f"Residuals plots saved to '{plot_dir}{residual_file}'")
    
def save_results_to_csv(results_matrix, nanopore_name, peptide_name, output_filepath='./results/kinetic_analysis_results.csv'):
    """
    Saves the kinetic analysis results (slope, intercept, r_squared) to a CSV file.

    Args:
        results_matrix (numpy.ndarray): Matrix of dictionaries from get_ln_survival_and_fit_matrix.
    ...
    if not results_list:
        print(f"No results to save for {nanopore_name} and {peptide_name}.")
        return
    ...
    if os.path.exists(output_filepath):
        ...
        print(f"Appended results to '{output_filepath}'")
    else:
        df.to_csv(output_filepath, mode='w', header=True, index=False)
        print(f"Created and saved results to '{output_filepath}'")
...
if __name__ == '__main__':
    ...
    output_csv_path = f'./results/{nanopore_name}_kinetic_analysis_results.csv'
    # Remove existing file to start fresh for each run
    ...
    for peptide_name in peptide_names:
        ...
        results_matrix = get_ln_survival_and_fit_matrix(dwell_matrix)
        
        # Plot and save results for the current peptide
        plot_ln_survival_and_residuals(results_matrix, nanopore_name, peptide_name)
        
        # Save the kinetic analysis results to the CSV file
        save_results_to_csv(results_matrix, nanopore_name, peptide_name, output_filepath=output_csv_path)
