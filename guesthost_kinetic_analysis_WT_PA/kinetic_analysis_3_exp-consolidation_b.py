...
def consolidate_best_models(results_1exp, results_2exp, results_3exp, peptide_name, nanopore_name, output_dir='./results/'):
    """
    Selects the best kinetic model for each transition based on the lowest BIC score,
    consolidates the results into a standardized table, and saves it to a CSV file.
    ...
    Args:
        ...
        peptide_name (str): The name of the peptide being analyzed.
        nanopore_name (str): The name of the nanopore used.
        output_dir (str): Directory to save the output CSV file.
    """
    print("\nConsolidating results to find the best model for each transition...")
    consolidated_data = []
    rows, cols = results_1exp.shape

    for i in range(rows):
        for j in range(cols):
            # Gather models with a valid, finite BIC score for the current transition
            models = {
            ...
            valid_models = {
                name: res for name, res in models.items() 
                if res is not None and 'bic' in res['fit'] and np.isfinite(res['fit']['bic'])
            }

            if not valid_models:
                continue # Skip this transition if no valid fits were found

            # Find the model with the minimum BIC score
            best_model_name = min(valid_models, key=lambda name: valid_models[name]['fit']['bic'])
            best_results = valid_models[best_model_name]
            fit_params = best_results['fit']

            # Initialize standardized component parameters
            tau_fast, A_fast = np.nan, np.nan
            tau_mid, A_mid = np.nan, np.nan
            tau_slow, A_slow = np.nan, np.nan

            # Unpack and sort parameters based on the best model type
            if best_model_name == '1-exp':
                tau_fast = fit_params.get('tau', np.nan)
                A_fast = fit_params.get('A', np.nan)
            
            elif best_model_name == '2-exp':
                components = sorted([(fit_params['tau1'], fit_params['A1']), 
                                     (fit_params['tau2'], fit_params['A2'])])
                tau_fast, A_fast = components[0]
                tau_slow, A_slow = components[1]

            elif best_model_name == '3-exp':
                components = sorted([(fit_params['tau1'], fit_params['A1']), 
                                     (fit_params['tau2'], fit_params['A2']),
                                     (fit_params['tau3'], fit_params['A3'])])
                tau_fast, A_fast = components[0]
                tau_mid, A_mid = components[1]
                tau_slow, A_slow = components[2]

            # Create a dictionary for the consolidated row
            row = {
                'peptide_name': peptide_name,
                'nanopore_name': nanopore_name,
                'transition_from': i,
                ...
                'tau_mean': fit_params['tau_mean'],
                'tau_mean_err': fit_params['tau_mean_err'],
                'tau_fast': tau_fast,
                'A_fast': A_fast,
                'tau_mid': tau_mid,
                'A_mid': A_mid,
    ...
    if not consolidated_data:
        print(f"No valid models found to consolidate for {peptide_name}.")
        return
    ...
    column_order = [
        'peptide_name', 'nanopore_name', 'transition_from', 'transition_to', 
        'best_model', 'bic', 'r_squared', 'tau_mean', 'tau_mean_err', 
        'tau_fast', 'A_fast', 'tau_mid', 'A_mid', 'tau_slow', 'A_slow'
    ]
    df = df[column_order]

    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, f'{nanopore_name}_{peptide_name}_best_models.csv')
    df.to_csv(output_path, index=False, float_format='%.6g')
    print(f"✅ Consolidated results for {peptide_name} saved to '{output_path}'")
...
if __name__ == '__main__':
    ...
    for peptide_name in peptide_names:
        ...
        # Consolidate the best exponential models per transition per peptide based on BIC logic 
        consolidate_best_models(
            results_matrix_1exp,
            results_matrix_2exp,
            results_matrix_3exp,
            peptide_name,
