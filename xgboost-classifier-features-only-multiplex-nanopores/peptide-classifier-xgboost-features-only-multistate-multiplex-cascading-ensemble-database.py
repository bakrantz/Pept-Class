...
if __name__ == "__main__":
    ...
    desired_processing_params = {
        ...
        'min_event_duration_ms': 15, # NEW: Reverting to a more robust threshold to filter out noisy events.
    ...
    # This classifier uses features from BOTH nanopores to get the best possible group split.
    ...
    # --- STEP 2: Train Level 2 Specialist Classifiers ---
    # We now train the Level 2 classifiers on ALL available data for their group
    # from BOTH nanopores, not just a single one. This should improve robustness.
    print("\n--- STEP 2: Training Level 2 Group-Specific Classifiers (using all data) ---")
    
    # Aromatic peptides: train on all aromatic data from both nanopores
    ...
    # Filter training data for aromatic peptides (no nanopore filter)
    aromatic_train_indices = np.isin(y_train_global, aromatic_classes)
    X_train_aromatic = X_train_global[aromatic_train_indices]
    y_train_aromatic = np.array([aromatic_label_map[label] for label in y_train_global[aromatic_train_indices]])

    # Non-Aromatic peptides: train on all non-aromatic data from both nanopores
    ...
    # Filter training data for non-aromatic peptides (no nanopore filter)
    non_aromatic_train_indices = np.isin(y_train_global, non_aromatic_classes)
    X_train_non_aromatic = X_train_global[non_aromatic_train_indices]
    y_train_non_aromatic = np.array([non_aromatic_label_map[label] for label in y_train_global[non_aromatic_train_indices]])

    # Train Aromatic Classifier (now on all data)
    xgbc_aromatic = xgb.XGBClassifier(
        objective='multi:softmax', num_class=len(aromatic_classes),
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='merror'
    )
    xgbc_aromatic.fit(X_train_aromatic, y_train_aromatic)
    print("Level 2 Aromatic classifier trained successfully.")

    # Train Non-Aromatic Classifier (now on all data)
    xgbc_non_aromatic = xgb.XGBClassifier(
        objective='multi:softmax', num_class=len(non_aromatic_classes),
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='merror'
    )
    xgbc_non_aromatic.fit(X_train_non_aromatic, y_train_non_aromatic)
    ...
    for i in range(len(X_test_global)):
        ...
        if group_pred == 1: # Aromatic group
            final_pred_level2 = xgbc_aromatic.predict(feature_vector)[0]
            final_pred = rev_aromatic_map[final_pred_level2]
        else: # Non-Aromatic group
            final_pred_level2 = xgbc_non_aromatic.predict(feature_vector)[0]
    ...
    print("\n--- Evaluation Metrics for the Hierarchical Ensemble ---")
    ...
    plot_filepath = os.path.join(plots_output_dir, 'peptide-classifier-xgboost-hierarchical-aromatic-nonaromatic-all-data-confusion_matrix.png')
