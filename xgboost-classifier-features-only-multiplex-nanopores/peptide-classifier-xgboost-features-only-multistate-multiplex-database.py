...
if __name__ == "__main__":
    ...
    # Nanopore names and encodings used in multiplexed analysis
    nanopore_names =  ['PA', 'PA_F427A', 'PA_F427Y']
    nanopore_names_encoding = {
        'PA': 0,
        'PA_F427A': 1,
        'PA_F427Y': 2,
    }
    ...
    # The following does not balance data for each nanopore;
    # however, the peptides are downsampled and balanced in load_translocation_data_from_database()
    # Does the nanopore name need to be numerically encoded for XGBoost model? I did that just in case.
    all_features = []
    all_labels = []
    for nanopore_name in nanopore_names:
        nanopore_encoding = nanopore_names_encoding[nanopore_name]
        # Database query to find proper raw peptide translocation stream files for each nanopore
        raw_db_query = {
            'experimental': True,
        ...
        updated_features = [subfeatures + [nanopore_encoding] for subfeatures in features]
        all_features.extend(updated_features)
        all_labels.extend(labels)
        
    if len(all_features) == 0: # Check if the list is empty
    ...
    plot_filepath = os.path.join(plots_output_dir, 'peptide-classifier-xgboost-multiplexed-confusion_matrix.png')
