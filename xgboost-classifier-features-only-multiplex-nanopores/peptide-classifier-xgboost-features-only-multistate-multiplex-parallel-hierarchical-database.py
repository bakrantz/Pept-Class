...
if __name__ == "__main__":
    ...
    # --- NEW STEP: Train Level 1 classifiers separately for each nanopore ---
    ...
    # --- STEP 2: Train Level 2 Specialist Classifiers, one for each nanopore and group ---
    print("\n--- STEP 2: Training Level 2 Nanopore-Specific & Group-Specific Classifiers ---")
    ...
    def filter_training_data(X_train, y_train, nanopore_train, nanopore_name, peptide_class_list):
        indices = np.logical_and(np.isin(y_train, peptide_class_list), nanopore_train == nanopore_name)
        return X_train[indices], y_train[indices]

    X_train_pa_aromatic, y_train_pa_aromatic = filter_training_data(
        X_train_global, y_train_global, nanopore_train, nanopore_pa_label, aromatic_classes
    )
    X_train_pa_non_aromatic, y_train_pa_non_aromatic = filter_training_data(
        X_train_global, y_train_global, nanopore_train, nanopore_pa_label, non_aromatic_classes
    )
    X_train_f427a_aromatic, y_train_f427a_aromatic = filter_training_data(
        X_train_global, y_train_global, nanopore_train, nanopore_f427a_label, aromatic_classes
    )
    X_train_f427a_non_aromatic, y_train_f427a_non_aromatic = filter_training_data(
        X_train_global, y_train_global, nanopore_train, nanopore_f427a_label, non_aromatic_classes
    )

    # Re-map labels for each specialist classifier to start from 0
    ...
    # Initialize and train the four specialist classifiers
    ...
    # --- STEP 3: Hierarchical Prediction and Evaluation on the Global Test Set ---
    print("\n--- STEP 3: Making Hierarchical Predictions ---")
    final_predictions = []
    ...
    for i in range(len(X_test_global)):
        ...
        if nanopore_source == nanopore_f427a_label:
            # First, classify the group using the PA F427A-specific Level 1 classifier
            group_pred = xgbc_f427a_level1.predict(feature_vector)[0]
            if group_pred == 1: # Aromatic group
                final_pred_level2 = xgbc_f427a_aromatic.predict(feature_vector)[0]
                final_pred = rev_aromatic_map[final_pred_level2]
            else: # Non-Aromatic group
                final_pred_level2 = xgbc_f427a_non_aromatic.predict(feature_vector)[0]
                final_pred = rev_non_aromatic_map[final_pred_level2]
        else: # Assumed to be PA WT
            # First, classify the group using the PA WT-specific Level 1 classifier
            group_pred = xgbc_pa_level1.predict(feature_vector)[0]
            if group_pred == 1: # Aromatic group
                final_pred_level2 = xgbc_pa_aromatic.predict(feature_vector)[0]
                final_pred = rev_aromatic_map[final_pred_level2]
            else: # Non-Aromatic group
                final_pred_level2 = xgbc_pa_non_aromatic.predict(feature_vector)[0]
                final_pred = rev_non_aromatic_map[final_pred_level2]
    ...
    print("\n--- Evaluation Metrics for the Parallel Hierarchical Ensemble ---")
    ...
    plot_filepath = os.path.join(plots_output_dir, 'peptide-classifier-xgboost-hierarchical-parallel-nanopore-specialist-confusion_matrix.png')
