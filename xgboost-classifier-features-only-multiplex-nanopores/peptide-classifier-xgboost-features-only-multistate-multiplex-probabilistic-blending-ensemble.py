...
if __name__ == "__main__":
    ...
    # Labels for the Level 1 classifier (Aromatic vs. Non-Aromatic)
    # 0 = Non-Aromatic, 1 = Aromatic
    ...
    group_labels_encoding = {
        peptide: (1 if peptide in aromatic_peptides else 0)
        for peptide in ordered_peptide_names
    }
    group_plot_labels = ['Non-Aromatic', 'Aromatic']
    
    nanopore_names_for_ensemble = ['PA', 'PA_F427A']
    nanopore_pa_label = 'PA'
    nanopore_f427a_label = 'PA_F427A'
    ...
    if len(nanopore_data_features) < 2:
        print("Not enough nanopore data to perform cascading ensemble. Exiting.")
        sys.exit(1)
    ...
    # Map original labels to new group labels for the Level 1 classifier
    all_group_labels = np.array([group_labels_encoding[ordered_peptide_names[label]] for label in all_labels])

    # Perform a single global train-test split for all models
    X_train_global, X_test_global, y_train_global, y_test_global, y_group_train, y_group_test, nanopore_train, nanopore_test = train_test_split(
        all_features, all_labels, all_group_labels, all_nanopore_labels,
    ...
    # --- STEP 1: Training Level 1 Aromatic/Non-Aromatic Classifiers (one per nanopore) ---
    print("\n--- STEP 1: Training Level 1 Aromatic/Non-Aromatic Classifiers (one per nanopore) ---")
    
    def filter_data_by_nanopore(X, y, nanopore_labels, nanopore_name):
        indices = nanopore_labels == nanopore_name
        return X[indices], y[indices]

    # Create nanopore-specific training data for Level 1 classifiers
    X_train_pa_level1, y_group_train_pa = filter_data_by_nanopore(X_train_global, y_group_train, nanopore_train, nanopore_pa_label)
    X_train_f427a_level1, y_group_train_f427a = filter_data_by_nanopore(X_train_global, y_group_train, nanopore_train, nanopore_f427a_label)

    # Train Level 1 classifier for PA WT
    xgbc_pa_level1 = xgb.XGBClassifier(
        objective='binary:logistic',
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='logloss'
    )
    xgbc_pa_level1.fit(X_train_pa_level1, y_group_train_pa)
    print("Level 1 PA WT classifier trained successfully.")

    # Train Level 1 classifier for PA F427A
    xgbc_f427a_level1 = xgb.XGBClassifier(
        objective='binary:logistic',
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='logloss'
    )
    xgbc_f427a_level1.fit(X_train_f427a_level1, y_group_train_f427a)
    print("Level 1 PA F427A classifier trained successfully.")


    # --- STEP 2: Augment training data with Level 1 probabilities and train Level 2 classifiers ---
    print("\n--- STEP 2: Augmenting data with Level 1 probabilities and training Level 2 classifiers ---")
    
    # Get Level 1 probabilities for the entire global training set
    proba_train_pa_global = xgbc_pa_level1.predict_proba(X_train_global)
    proba_train_f427a_global = xgbc_f427a_level1.predict_proba(X_train_global)

    def filter_and_augment_data(X_train, y_train, nanopore_train, nanopore_name, peptide_class_list, proba_data_global):
        indices = np.logical_and(np.isin(y_train, peptide_class_list), nanopore_train == nanopore_name)
        X_filtered = X_train[indices]
        y_filtered = y_train[indices]
        probas_for_filtered_data = proba_data_global[indices]
        X_augmented = np.hstack((X_filtered, probas_for_filtered_data))
        return X_augmented, y_filtered

    # Define the class subsets
    aromatic_classes = [peptide_labels_encoding[p] for p in aromatic_peptides]
    non_aromatic_classes = [peptide_labels_encoding[p] for p in non_aromatic_peptides]
    
    # Augment and map labels for each specialist classifier
    aromatic_label_map = {peptide_labels_encoding[p]: i for i, p in enumerate(aromatic_peptides)}
    non_aromatic_label_map = {peptide_labels_encoding[p]: i for i, p in enumerate(non_aromatic_peptides)}
    
    # PA WT
    X_train_pa_aromatic, y_train_pa_aromatic = filter_and_augment_data(X_train_global, y_train_global, nanopore_train, nanopore_pa_label, aromatic_classes, proba_train_pa_global)
    y_train_pa_aromatic_mapped = np.array([aromatic_label_map[label] for label in y_train_pa_aromatic])
    
    X_train_pa_non_aromatic, y_train_pa_non_aromatic = filter_and_augment_data(X_train_global, y_train_global, nanopore_train, nanopore_pa_label, non_aromatic_classes, proba_train_pa_global)
    y_train_pa_non_aromatic_mapped = np.array([non_aromatic_label_map[label] for label in y_train_pa_non_aromatic])

    # PA F427A
    X_train_f427a_aromatic, y_train_f427a_aromatic = filter_and_augment_data(X_train_global, y_train_global, nanopore_train, nanopore_f427a_label, aromatic_classes, proba_train_f427a_global)
    y_train_f427a_aromatic_mapped = np.array([aromatic_label_map[label] for label in y_train_f427a_aromatic])
    
    X_train_f427a_non_aromatic, y_train_f427a_non_aromatic = filter_and_augment_data(X_train_global, y_train_global, nanopore_train, nanopore_f427a_label, non_aromatic_classes, proba_train_f427a_global)
    y_train_f427a_non_aromatic_mapped = np.array([non_aromatic_label_map[label] for label in y_train_f427a_non_aromatic])

    # Initialize and train the four specialist classifiers on the augmented data
    xgbc_pa_aromatic = xgb.XGBClassifier(
        objective='multi:softmax', num_class=len(aromatic_classes),
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='merror'
    )
    xgbc_pa_aromatic.fit(X_train_pa_aromatic, y_train_pa_aromatic_mapped)
    print("Level 2 PA Aromatic classifier trained successfully.")

    xgbc_pa_non_aromatic = xgb.XGBClassifier(
        objective='multi:softmax', num_class=len(non_aromatic_classes),
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='merror'
    )
    xgbc_pa_non_aromatic.fit(X_train_pa_non_aromatic, y_train_pa_non_aromatic_mapped)
    print("Level 2 PA Non-Aromatic classifier trained successfully.")

    xgbc_f427a_aromatic = xgb.XGBClassifier(
        objective='multi:softmax', num_class=len(aromatic_classes),
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='merror'
    )
    xgbc_f427a_aromatic.fit(X_train_f427a_aromatic, y_train_f427a_aromatic_mapped)
    print("Level 2 PA F427A Aromatic classifier trained successfully.")

    xgbc_f427a_non_aromatic = xgb.XGBClassifier(
        objective='multi:softmax', num_class=len(non_aromatic_classes),
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='merror'
    )
    xgbc_f427a_non_aromatic.fit(X_train_f427a_non_aromatic, y_train_f427a_non_aromatic_mapped)
    print("Level 2 PA F427A Non-Aromatic classifier trained successfully.")


    # --- STEP 3: Train and use a meta-classifier for final predictions ---
    print("\n--- STEP 3: Training a meta-classifier for final predictions ---")
    
    # Create reverse maps for converting Level 2 predictions back to original labels
    rev_aromatic_map = {i: label for label, i in aromatic_label_map.items()}
    rev_non_aromatic_map = {i: label for label, i in non_aromatic_label_map.items()}

    def get_blending_features(X, nanopore_labels,
                              xgbc_pa_level1, xgbc_pa_aromatic, xgbc_pa_non_aromatic,
                              xgbc_f427a_level1, xgbc_f427a_aromatic, xgbc_f427a_non_aromatic):
        """Generates the feature vectors for the meta-classifier."""
        meta_features = []
        for i in range(len(X)):
            feature_vector = X[i].reshape(1, -1)
            nanopore_source = nanopore_labels[i]

            if nanopore_source == nanopore_f427a_label:
                # Get Level 1 probabilities and augment the feature vector
                proba_vector = xgbc_f427a_level1.predict_proba(feature_vector)
                augmented_feature_vector = np.hstack((feature_vector, proba_vector))

                # Get Level 2 probabilities from BOTH specialists
                probas_aromatic = xgbc_f427a_aromatic.predict_proba(augmented_feature_vector).flatten()
                probas_non_aromatic = xgbc_f427a_non_aromatic.predict_proba(augmented_feature_vector).flatten()
                
            else: # Assumed to be PA WT
                # Get Level 1 probabilities and augment the feature vector
                proba_vector = xgbc_pa_level1.predict_proba(feature_vector)
                augmented_feature_vector = np.hstack((feature_vector, proba_vector))

                # Get Level 2 probabilities from BOTH specialists
                probas_aromatic = xgbc_pa_aromatic.predict_proba(augmented_feature_vector).flatten()
                probas_non_aromatic = xgbc_pa_non_aromatic.predict_proba(augmented_feature_vector).flatten()

            # The order of the blended features must be consistent
            meta_features.append(np.hstack((probas_non_aromatic, probas_aromatic)))
        return np.array(meta_features)

    # Generate meta-features for training the final classifier
    X_meta_train = get_blending_features(X_train_global, nanopore_train,
                                         xgbc_pa_level1, xgbc_pa_aromatic, xgbc_pa_non_aromatic,
                                         xgbc_f427a_level1, xgbc_f427a_aromatic, xgbc_f427a_non_aromatic)
    
    # Train the final meta-classifier on the Level 2 probabilities
    xgbc_meta = xgb.XGBClassifier(
        objective='multi:softmax', num_class=len(ordered_peptide_names),
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='merror'
    )
    xgbc_meta.fit(X_meta_train, y_train_global)
    print("Meta-classifier trained successfully.")

    # Generate meta-features for the test set
    X_meta_test = get_blending_features(X_test_global, nanopore_test,
                                        xgbc_pa_level1, xgbc_pa_aromatic, xgbc_pa_non_aromatic,
                                        xgbc_f427a_level1, xgbc_f427a_aromatic, xgbc_f427a_non_aromatic)

    # Predict with the meta-classifier
    final_predictions = xgbc_meta.predict(X_meta_test)

    # --- Final Evaluation Metrics for the Blending Ensemble ---
    ...
    plot_filepath = os.path.join(plots_output_dir, 'peptide-classifier-xgboost-hierarchical-blending-nanopore-specialist-confusion_matrix.png')
