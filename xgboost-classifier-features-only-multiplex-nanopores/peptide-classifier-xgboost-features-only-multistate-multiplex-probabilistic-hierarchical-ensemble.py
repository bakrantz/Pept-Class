...
if __name__ == "__main__":
    ...
    # Get Level 1 probabilities for the entire training set
    # This is a key change: predict probabilities on the whole training set for correct indexing
    ...
    # Filter training data for each of the four combinations
    def filter_and_augment_data(X_train, y_train, nanopore_train, nanopore_name, peptide_class_list, proba_data_global):
        # Create a single combined index to filter all arrays correctly
    ...
    # --- STEP 3: Probabilistic Hierarchical Prediction and Evaluation ---
    print("\n--- STEP 3: Making Probabilistic Hierarchical Predictions ---")
    final_predictions = []
    ...
    for i in range(len(X_test_global)):
        feature_vector = X_test_global[i].reshape(1, -1) # Reshape for single sample prediction
        nanopore_source = nanopore_test[i]

        if nanopore_source == nanopore_f427a_label:
            proba_vector = xgbc_f427a_level1.predict_proba(feature_vector)
            group_pred = np.argmax(proba_vector)
            
            # Augment feature vector with the probabilities
            augmented_feature_vector = np.hstack((feature_vector, proba_vector))

            if group_pred == 1: # Aromatic group
                final_pred_level2 = xgbc_f427a_aromatic.predict(augmented_feature_vector)[0]
                final_pred = rev_aromatic_map[final_pred_level2]
            else: # Non-Aromatic group
                final_pred_level2 = xgbc_f427a_non_aromatic.predict(augmented_feature_vector)[0]
                final_pred = rev_non_aromatic_map[final_pred_level2]
            ...
            if group_pred == 1: # Aromatic group
                final_pred_level2 = xgbc_pa_aromatic.predict(augmented_feature_vector)[0]
                final_pred = rev_aromatic_map[final_pred_level2]
            else: # Non-Aromatic group
                final_pred_level2 = xgbc_pa_non_aromatic.predict(augmented_feature_vector)[0]
                final_pred = rev_non_aromatic_map[final_pred_level2]
    ...
    # --- Final Evaluation Metrics for the Hierarchical Model ---
    print("\n--- Evaluation Metrics for the Probabilistic Hierarchical Ensemble ---")
    ...
    # Save figure of the confusion matrix for the final hierarchical model
    plot_filepath = os.path.join(plots_output_dir, 'peptide-classifier-xgboost-hierarchical-probabilistic-nanopore-specialist-confusion_matrix.png')
