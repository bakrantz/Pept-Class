...
if __name__ == "__main__":
    ...
    # --- STEP 1: Train the Level 1 (Aromatic vs. Non-Aromatic) Classifier ---
    print("\n--- STEP 1: Training Level 1 Aromatic/Non-Aromatic Classifier ---")
    xgbc_level1 = xgb.XGBClassifier(
        objective='binary:logistic',
        n_estimators=1000, learning_rate=0.05, max_depth=5,
        random_state=random_state, n_jobs=-1, eval_metric='logloss'
    )
    xgbc_level1.fit(X_train_global, y_group_train)
    print("Level 1 classifier trained successfully.")
    ...
    # Use the Level 1 classifier to predict the group for each test sample
    group_predictions = xgbc_level1.predict(X_test_global)

    for i in range(len(X_test_global)):
        feature_vector = X_test_global[i].reshape(1, -1) # Reshape for single sample prediction
        group_pred = group_predictions[i]
    ...
    print("\n--- Evaluation Metrics for the Nanopore-Specialist Hierarchical Ensemble ---")
    ...
    plot_filepath = os.path.join(plots_output_dir, 'peptide-classifier-xgboost-hierarchical-nanopore-specialist-confusion_matrix.png')
