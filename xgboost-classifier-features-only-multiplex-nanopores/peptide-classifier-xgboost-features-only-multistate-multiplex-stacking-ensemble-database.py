...
if __name__ == "__main__":
    ...
    # Nanopore names to use in the ensemble
    # This list can be modified to include 2 or 3 nanopores
    nanopore_names_for_ensemble = ['PA', 'PA_F427A']
    # nanopore_names_for_ensemble = ['PA', 'PA_F427A', 'PA_F427Y']

    # 2. Load and prepare data for each individual nanopore (Base Models)
    random_state = 42  # Keep consistent random state for comparison
    ...
    desired_processing_params = {
        ...
        'min_event_duration_ms': 20,  # Events greater than or equal to this value will be included
    ...
    # Dictionary to hold features and labels for each nanopore
    nanopore_data = {}
    for nanopore_name in nanopore_names_for_ensemble:
        ...
        features, labels = load_translocation_data_from_database(
        ...
        if len(features) == 0:
            print(f"Data loading failed for {nanopore_name}. Skipping.")
            continue
        nanopore_data[nanopore_name] = {'features': features, 'labels': labels}

    # Check if we have data for at least two nanopores
    if len(nanopore_data) < 2:
        print("Not enough nanopore data to perform stacking. Please ensure data exists for at least two nanopores.")
        sys.exit(1)

    # --- STEP 1: Perform a single global train-test split on all events ---
    # Concatenate all data from all nanopores for the single global split
    all_features_list = []
    all_labels_list = []
    for nanopore_name in nanopore_data:
        all_features_list.append(nanopore_data[nanopore_name]['features'])
        all_labels_list.append(nanopore_data[nanopore_name]['labels'])

    all_features = np.concatenate(all_features_list, axis=0)
    all_labels = np.concatenate(all_labels_list, axis=0)

    # This is the SINGLE, GLOBAL train-test split that all models will use.
    ...
    print(f"\nTotal training events for all models: {len(y_train_global)}")
    print(f"Total testing events for all models: {len(y_test_global)}")

    # --- STEP 2: Train Base Models on the global training data ---
    base_models = {}
    print("\n--- STEP 2: Training Base-Level Classifiers on global training data ---")
    for nanopore_name in nanopore_names_for_ensemble: # Use the original list to ensure order
        if nanopore_name in nanopore_data:
            print(f"Training XGBoost base model for {nanopore_name}...")
            # All base models are now trained on the same global training set.
            # This ensures that the predictions for meta-features will have the same dimensions.
            ...
            xgbc_base.fit(X_train_global, y_train_global)
            base_models[nanopore_name] = xgbc_base
            print(f"Base model for {nanopore_name} trained.")

    # --- STEP 3: Generate Meta-Features (Probabilities from Base Models) ---
    print("\n--- STEP 3: Generating Meta-Features ---")
    # List to store the probability predictions for the training set
    X_meta_train_list = []
    # List to store the probability predictions for the test set
    X_meta_test_list = []
    for nanopore_name in nanopore_names_for_ensemble:
        if nanopore_name in base_models:
            print(f"Generating predictions from {nanopore_name} model...")
            model = base_models[nanopore_name]
            # Get probability predictions for both the global train and global test sets
            y_prob_train = model.predict_proba(X_train_global)
            y_prob_test = model.predict_proba(X_test_global)

            X_meta_train_list.append(y_prob_train)
            X_meta_test_list.append(y_prob_test)

    # Stack the probability predictions horizontally to form the new meta-features
    # This will now work because all arrays in the lists have the same number of rows.
    X_meta_train = np.hstack(X_meta_train_list)
    X_meta_test = np.hstack(X_meta_test_list)

    print(f"Meta-features created. Training set shape: {X_meta_train.shape}, Test set shape: {X_meta_test.shape}")

    # --- STEP 4: Train the Meta-Model (Final Classifier) ---
    print("\n--- STEP 4: Training the Final Meta-Classifier ---")
    xgbc_meta = xgb.XGBClassifier(
        objective='multi:softmax',
        num_class=len(ordered_peptide_names),
        n_estimators=500,  # A smaller model is often sufficient for the meta-classifier
        learning_rate=0.1,
        max_depth=3,  # A simpler model to avoid overfitting on the predictions
        random_state=random_state,
        n_jobs=-1,
        eval_metric='merror'
    )
    # The meta-model is trained on the meta-features and the global training labels
    xgbc_meta.fit(X_meta_train, y_train_global)
    print("Meta-model training complete.")

    # --- 5. Make predictions and evaluate the Meta-Model ---
    # The meta-model predicts on the meta-features generated from the global test set.
    predictions = xgbc_meta.predict(X_meta_test)

    # --- Evaluation Metrics for the Meta-Model ---
    print("\n--- Evaluation Metrics for the Stacking Ensemble Meta-Model ---")
    ...
    print(classification_report(y_test_global, predictions, target_names=ordered_peptide_names, zero_division=0))
    accuracy_peptide = accuracy_score(y_test_global, predictions)
    print(f"\nOverall Peptide Classification Accuracy: {accuracy_peptide:.4f}")
    precision_macro_peptide = precision_score(y_test_global, predictions, average='macro', zero_division=0)
    recall_macro_peptide = recall_score(y_test_global, predictions, average='macro', zero_division=0)
    f1_macro_peptide = f1_score(y_test_global, predictions, average='macro', zero_division=0)
    ...
    # Save figure of the confusion matrix for the meta-model
    plot_filepath = os.path.join(plots_output_dir, 'peptide-classifier-xgboost-stacking-confusion_matrix.png')
