...
def load_peptide_data_with_features(peptide_data_paths, peptide_labels_encoding, test_size=0.2, random_state=42):
    ...
    # Ensure the order of features matches your training data consistently
    feature_keys = [
    ...
    for event in all_events_data:
        ...
        features_list = [event.get(key, 0) for key in feature_keys] # Use .get() with default 0 to handle missing keys gracefully
...
if __name__ == "__main__":
    ...
    visualize_confusion_matrix(cm, peptide_plot_labels, filename="./plots/peptide-classifier-xgboost-confusion_matrix.png")
    print(f"\nConfusion matrix plot saved to ./plots/peptide-classifier-xgboost-confusion_matrix.png")
