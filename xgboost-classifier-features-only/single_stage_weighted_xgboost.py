...
def load_translocation_data_from_database(
    ...
    if downsample_to_min_events:
        ...
        # Logic remains intact but is bypassed by the False default flag
...
def visualize_confusion_matrix(confusion_matrix, class_names, filename="confusion_matrix.png"):
    ...
    NOTE: Using normalize='true' during cm generation passes values 0-1, so we format as .2f
    """
    plt.figure(figsize=(9, 8))
    sns.heatmap(confusion_matrix, annot=True, fmt=".2f", cmap="Blues", vmin=0, vmax=1,
                xticklabels=class_names, yticklabels=class_names,
                annot_kws={"size": 14})
    plt.xlabel('Predicted Peptide', fontsize=16, fontweight='bold')
    plt.ylabel('True Peptide', fontsize=16, fontweight='bold')
    plt.title('Peptide Classification Confusion Matrix (Row-Normalized)', fontsize=16, fontweight='bold', pad=15)
...
if __name__ == "__main__":
    ...
    # NEW: Passing downsample_to_min_events = False
    ...
    # NEW: Calculate mathematically balanced weights based on the training set distribution
    ...
    # NEW: Pass the sample weights into the fit method
    ...
    # NEW: Row-normalize the confusion matrix so percentages (0 to 1) are shown, which handles the visual bias of uneven data
    ...
    plot_filepath = os.path.join(plots_output_dir, 'peptide-classifier-xgboost-confusion_matrix_weighted_full_data.png')
