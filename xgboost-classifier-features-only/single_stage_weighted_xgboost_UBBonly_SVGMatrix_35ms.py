...
import sys
import json
import csv  # Added for CSV file export
import matplotlib
...
from sklearn.utils.class_weight import compute_sample_weight # NEW: For handling imbalanced classes
import pickle
import matplotlib as mpl

# --- Global Font Settings for SVG Export ---
# Crucial for editable fonts in Inkscape
mpl.rcParams['svg.fonttype'] = 'none'
plt.rcParams['font.family'] = 'sans-serif' # Use 'sans-serif' as a generic family
...
def load_translocation_data_from_database(
    ...
    random_state: int = 42,
    downsample_to_min_events: bool = False # DEFAULT CHANGED TO FALSE
...
def visualize_confusion_matrix(confusion_matrix, class_names, filename="confusion_matrix.png"):
    """
    Visualizes a confusion matrix as a color-coded heatmap.
    Optimized for 20x20 multiclass layouts to ensure clear text visibility.
    """
...
if __name__ == "__main__":
    ...
    # Plot labels for 20 natural guest amino acids in alphabetical order according to three-letter notion
    peptide_plot_labels = [
        'Ala', 'Arg', 'Asn', 'Asp', 'Cys', 'Gln', 'Glu', 'Gly', 'His', 'Ile', 
        'Leu', 'Lys', 'Met', 'Phe', 'Pro', 'Ser', 'Thr', 'Trp', 'Tyr', 'Val'
    ]

    # Build peptide_labels_encoding dynamically from peptide_plot_labels
    ...
    raw_db_query = {
        ...
        'buffer': 'UBB' #only Jaylen's experiments
    }

    # Define the list of minimum duration cutoffs to evaluate
    duration_cutoffs = [35]
    random_state = 42 

    # Summary tracking storage initialization
    summary_results = []
    ...
    for cutoff in duration_cutoffs:
        ...
        desired_processing_params = {
            ...
            'min_event_duration_ms': cutoff,  # Updated dynamically by the loop
        ...
        # Passing downsample_to_min_events = False
        ...
        # Calculate mathematically balanced weights based on the training set distribution
        print("\nComputing sample weights to handle class imbalance...")
        sample_weights = compute_sample_weight(class_weight='balanced', y=y_train)

        xgbc = xgb.XGBClassifier(
            objective='multi:softprob', # softprob is generally safer for imbalanced multiclass
        ...
        # Pass the sample weights into the fit method
        ...
        # Row-normalize the confusion matrix so percentages (0 to 1) are shown
        cm_normalized = confusion_matrix(y_test, predictions, normalize='true')
        cm_raw = confusion_matrix(y_test, predictions)
        
        print("\nPeptide Classification Confusion Matrix (Raw Counts) - Test Set:")
        print(cm_raw)
        ...
        print(f"\nOverall Peptide Classification Accuracy ({cutoff} ms): {accuracy_peptide:.4f}")
        ...
        print(f"[SUCCESS] Results exported to: {txt_filename}")
        ...
        # Modified output filename to include the specific cutoff duration string
        ...
        svg_filepath = os.path.splitext(plot_filename)[0] + ".svg"
        ...
        print(f"Confusion matrix plot saved to {svg_filepath}\n")
